...
class Patrimoniomodel extends Models{
    ...
    public function listar($id){
        ...
        $query = $this->db->connect()->prepare('SELECT "Bien" AS clase, b.idbien AS "id", b.valorbien AS "valor", b.idtipobien, tb.nombre AS "tipo", b.idtipomoneda, b.idmodelo, p.patliquitotal, p.deudatotal, p.patbrutototal FROM '.$this->tablabien.' b JOIN '.$this->tablatipobien.' tb ON tb.idtipobien = b.idtipobien JOIN '.$this->tablausuariobien.' ub ON ub.idbien = b.idbien JOIN '.$this->tablapatrimonio.' p ON p.idpatrimonio = ub.idpatrimonio WHERE p.iddeclaracion = ? UNION SELECT "Deuda" AS clase, d.iddeuda AS "id", d.valordeuda AS "valor", d.idtipodeuda, td.nombre AS "tipo", d.idtipomoneda, d.idmodelo, p.patliquitotal, p.deudatotal, p.patbrutototal FROM '.$this->tabladeuda.' d JOIN '.$this->tablatipodeuda.' td ON td.idtipodeuda = d.idtipodeuda JOIN '.$this->tablausuariodeuda.' ud ON ud.iddeuda = d.iddeuda JOIN '.$this->tablapatrimonio.' p ON p.idpatrimonio = ud.idpatrimonio WHERE p.iddeclaracion = ?;');
        $query->execute([$id, $id]);
